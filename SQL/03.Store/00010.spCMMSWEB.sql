...
ALTER PROCEDURE [dbo].[spCMMSWEB]
	@sDanhMuc NVARCHAR(500) = N'DELETE_WORK_ORDER',
	...
	@CoAll INT = 0,
	@fcot1 FLOAT = '-1',
	@sCot1 NVARCHAR(500) = 'WO-202302000001',
	@sCot2 NVARCHAR(500) = '01.01',
	@sCot3 NVARCHAR(500) = '',
...
LEFT JOIN #YC T4 ON T1.MS_MAY = T4.MS_MAY
 ...
 SELECT T.MS_MAY,
		...
		ISNULL(TREYC,0) TREYC,
              ...
              ISNULL(TREGS,0)   TREGS,T.MUC_YC,T.MUC_BT,T.MUC_GS FROM (
		SELECT MS_MAY,
               TEN_MAY,
               YC,
               PBT,
		...
		UNION
	SELECT A.MS_MAY,A.TEN_MAY,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL FROM #MAY_USER A WHERE NOT EXISTS (SELECT * FROM #DSECOMAINT B WHERE A.MS_MAY = B.MS_MAY)
...
IF(@sDanhMuc = 'GET_WORDORDER_DETAILS')
	BEGIN
		SELECT DISTINCT A.MS_PHIEU_BAO_TRI,A.MS_BO_PHAN,C.TEN_BO_PHAN,A.MS_CV,D.MO_TA_CV,B.MS_PT,E.TEN_PT,F.MS_VI_TRI_PT,B.SL_TT ,REPLACE(CTTBCV.PATH_HD,'27.74.240.29','192.168.2.8') AS PATH_HD
...
BEGIN
	...
	BEGIN
		...
		SELECT DISTINCT B.MS_BO_PHAN AS MA_BP,C.TEN_BO_PHAN AS TEN_BP FROM dbo.PHIEU_BAO_TRI A
	...
	SELECT DISTINCT B.MS_PT,C.TEN_PT FROM dbo.PHIEU_BAO_TRI A
		INNER JOIN dbo.PHIEU_BAO_TRI_CONG_VIEC_PHU_TUNG B ON B.MS_PHIEU_BAO_TRI = A.MS_PHIEU_BAO_TRI AND a.MS_MAY = @deviceID
...
BEGIN
	SELECT TOP 100 MS_DH_NHAP_PT,b.DANG_NHAP_VIET AS DANG_NHAP,a.NGAY,c.TEN,a.LOCK FROM dbo.IC_DON_HANG_NHAP a
...
BEGIN
	...
	BEGIN TRY
		...
		if(LOWER(@UserName) = (SELECT LOWER(USERNAME_NGUOI_LAP) FROM dbo.PHIEU_BAO_TRI WHERE MS_PHIEU_BAO_TRI = @UserName))
		BEGIN
			SELECT 0 AS MA, N'Bạn không phải là người lập nên không có quyền xóa' AS [NAME]
		...
		WHERE A.MS_PHIEU_BAO_TRI = @sCot1)
		BEGIN
			SELECT 0 AS MA, N'Phiếu bảo trì này đã khai báo thời gian làm việc không được xóa!' AS [NAME]
			ROLLBACK TRANSACTION DWork;   
			RETURN;
		END
		END

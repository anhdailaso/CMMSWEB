...
INNER JOIN (SELECT DISTINCT MS_PHIEU_BAO_TRI, MS_PT, SUM(SL_TT) AS SL_TREN_PBT
 ...
 A_1.MS_PT = B_1.MS_PT
  ...
  LEFT JOIN
(
SELECT     dbo.PHIEU_BAO_TRI_CONG_VIEC_PHU_TUNG.MS_PT, SUM(dbo.PHIEU_BAO_TRI_CONG_VIEC_PHU_TUNG.SL_TT) AS SL_PBT_KHAC_1
...
GROUP BY dbo.PHIEU_BAO_TRI_CONG_VIEC_PHU_TUNG.MS_PT
  ...
  LEFT JOIN
(
SELECT     dbo.PHIEU_BAO_TRI_CONG_VIEC_PHU_TUNG.MS_PT, SUM(dbo.PHIEU_BAO_TRI_CONG_VIEC_PHU_TUNG.SL_TT) AS SL_PBT_KHAC_2
FROM         dbo.PHIEU_BAO_TRI AS A INNER JOIN
                          (SELECT DISTINCT MS_MAY
                            FROM          dbo.MashjGetMayNhaXuong('-1',
                                                       (SELECT     TOP (1) MS_N_XUONG

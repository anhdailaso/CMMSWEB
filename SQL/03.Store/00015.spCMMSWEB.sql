...
SELECT DISTINCT MS_MAY,T1.TEN_MAY,CASE WHEN CONVERT(DATE,MAX(NGAY_KE)) < CONVERT(DATE,GETDATE()) THEN 2 ELSE 1 END TREGS
...
BEGIN
		...
		DECLARE @ID_CHA BIGINT = (SELECT TOP 1 ID FROM dbo.THOI_GIAN_DUNG_MAY A INNER JOIN dbo.NGUYEN_NHAN_DUNG_MAY B ON B.MS_NGUYEN_NHAN = A.MS_NGUYEN_NHAN INNER JOIN dbo.DownTimeType C ON C.ID_DownTime = B.DownTimeTypeID WHERE MS_MAY = @deviceID AND DEN_GIO = @dCot1);
		...
		SELECT @deviceID = MS_MAY FROM dbo.PHIEU_BAO_TRI

		INSERT INTO dbo.THOI_GIAN_DUNG_MAY(MS_MAY,CaID,TU_GIO,TU_GIO_GOC,DEN_GIO,DEN_GIO_GOC,MS_NGUYEN_NHAN,GHI_CHU,USERNAME,THOI_GIAN_SUA_CHUA,THOI_GIAN_SUA,ID_CHA,MS_NGUYEN_NHAN_GOC,NGAY_DUNG,MS_HE_THONG,MS_PBT)
		VALUES(@deviceID,@ID_CA,@dCot1,@dCot1,@dCot2,@dCot2,@icot1,'WO',@UserName,CONVERT(DECIMAL(18,2),DATEDIFF(SECOND,@dCot1,@dCot2))/60,CONVERT(DECIMAL(18,2),DATEDIFF(SECOND,@dCot1,@dCot2))/60,@ID_CHA,@icot1,@NGAY_DUNG,(SELECT TOP 1 MS_HE_THONG FROM dbo.MAY_HE_THONG WHERE MS_MAY = @deviceID),@sCot1);
...
BEGIN
		...
		INSERT INTO dbo.THOI_GIAN_DUNG_MAY(MS_MAY,CaID,TU_GIO,TU_GIO_GOC,DEN_GIO,DEN_GIO_GOC,MS_NGUYEN_NHAN,GHI_CHU,USERNAME,THOI_GIAN_SUA_CHUA,THOI_GIAN_SUA,ID_CHA,MS_NGUYEN_NHAN_GOC,NGAY_DUNG,MS_HE_THONG,MS_PBT,UserEdit,DateEdit,CLOSED)
...
CONVERT(DATE,(CASE (SELECT TOP 1 CA_DEM FROM dbo.CA WHERE STT = ID_CA) WHEN 1 THEN CONVERT(DATE, DATEADD(HOUR, -(DATEPART(HOUR, (SELECT TOP 1 DEN_GIO FROM dbo.CA WHERE STT = ID_CA)) + (DATEPART(MINUTE, (SELECT TOP 1 DEN_GIO FROM dbo.CA WHERE STT = ID_CA))/60)), TU_GIO )) ELSE TU_GIO END)),
	(SELECT TOP 1 C.MS_HE_THONG FROM dbo.MAY_HE_THONG C WHERE MS_MAY = @deviceID),@sCot1,@UserName,GETDATE(),1
		FROM #BTNNM B
...
END
	...
	BEGIN
		...
		FROM NGUYEN_NHAN_DUNG_MAY A  WHERE A.BTDK = 1
		ORDER BY [Text]
	END
	ELSE
	BEGIN
	...
	INNER JOIN dbo.LOAI_MAY_NGUYEN_NHAN B ON B.MS_NGUYEN_NHAN = A.MS_NGUYEN_NHAN
	WHERE B.MS_LOAI_MAY = dbo.MGetLoaiMay(@deviceID) AND A.BTDK = 1
	ORDER BY [Text]

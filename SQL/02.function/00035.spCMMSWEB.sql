ALTER PROCEDURE [dbo].[spCMMSWEB]
	@sDanhMuc NVARCHAR(500) = N'SAVE_NGUOI_THU_HIEN',
	@DNgay DATE = '08/25/2023',
	@UserName NVARCHAR(255) = 'chauhongan',
	...
	@fcot2 FLOAT = '-1',
	@fcot3 FLOAT = '-1',
	@sCot1 NVARCHAR(500) = 'WO-202309000014',
	@sCot2 NVARCHAR(500) = '',
	@sCot3 NVARCHAR(500) = '',
	@sCot4 NVARCHAR(500) = '',
	@sCot5 NVARCHAR(500) = '',
	...
	@dCot3 DATETIME = '04/18/2023',
	@json NVARCHAR(MAX) = '[{"MS_CONG_NHAN":"XN-001"}]',
	@deviceID NVARCHAR(50) ='DMAN-002',
	@isDue INT = 1,
    @stt INT = 165
	--,
...
AND (T1.MS_PHIEU_BAO_TRI IN (SELECT MS_PHIEU_BAO_TRI FROM dbo.PHIEU_BAO_TRI_NHAN_SU WHERE MS_CONG_NHAN = ( SELECT TOP 1 MS_CONG_NHAN FROM dbo.USERS WHERE USERNAME = @UserName )) OR @bcot2 = 0)

--spCMMSWeb 

SELECT DISTINCT T2.MS_MAY,T3.TEN_MAY,T1.SO_PHIEU,CASE WHEN CONVERT(DATE,T1.NGAY_KH) < CONVERT(DATE,GETDATE()) THEN	1 ELSE 0 END TREGS,T1.MUC_UU_TIEN AS MUC_GS 
INTO #TMPGSTT
FROM dbo.GIAM_SAT_TINH_TRANG AS T1
INNER JOIN dbo.GIAM_SAT_TINH_TRANG_TS T2 ON T2.STT = T1.STT
INNER JOIN #MAY_USER T3 ON T3.MS_MAY = T2.MS_MAY
WHERE (CONVERT(DATE,T1.NGAY_KH) BETWEEN DATEADD(YEAR,-1,@DNgay) AND CONVERT(DATE,@DNgay)) AND (T1.HOAN_THANH = 0) 
AND (T1.STT IN (SELECT STT_GSTT FROM dbo.GIAM_SAT_TINH_TRANG_NV WHERE MS_CONG_NHAN = (SELECT TOP 1 MS_CONG_NHAN FROM dbo.USERS WHERE USERNAME = @UserName )) OR @bcot2 = 0)


--SELECT DISTINCT MS_MAY,T1.TEN_MAY,CASE WHEN CONVERT(DATE,MIN(NGAY_KE)) <= CONVERT(DATE,GETDATE()) THEN 2 ELSE 1 END TREGS
--INTO #GSTT FROM [dbo].[MGetHieuChuanKeGSTT](DATEADD(MONTH,-1,@DNgay),@DNgay,@UserName,@MsNXuong,-1,-1,@sCot1,'-1',-1,1,@NNgu) T1
--WHERE @bcot2 = 0
...
SELECT DISTINCT A.MS_MAY,A.TEN_MAY,((SELECT T.SO_PHIEU,T.TREGS,T.MUC_GS  FROM #TMPGSTT T WHERE T.MS_MAY = A.MS_MAY   FOR JSON AUTO )) AS sListGS
INTO #GSTT
FROM #TMPGSTT A
GROUP BY A.MS_MAY,A.TEN_MAY


SELECT DISTINCT T1.MS_MAY,T1.TEN_MAY,T4.sListYC,T2.sListBT,T3.sListGS
INTO #DSECOMAINT
FROM (
 ...
 BEGIN
		SELECT MS_MAY,
             ...
             ISNULL(sListYC,'') sListYC,
              ISNULL(sListGS,'') sListGS,
			  CASE WHEN (SELECT COUNT(*) FROM dbo.CAU_TRUC_THIET_BI_TS_GSTT T WHERE T.MS_MAY = A.MS_MAY) > 0 THEN 1 ELSE 0 END COGS
			 FROM #DSECOMAINT A
			 ORDER BY MS_MAY
 END
 ELSE
...
IF(@sDanhMuc = 'GET_MORNINGTORING')
BEGIN 
		--kiểm tra user đó có quyền trên menu phiếu bảo trì không
	SET @icot1  = (SELECT TOP 1 GROUP_ID FROM dbo.USERS WHERE USERNAME = @UserName)
	SET @icot2 = (SELECT COUNT(*) FROM dbo.NHOM_MENU WHERE GROUP_ID = @icot1 AND MENU_ID = 'mnuGiamsattinhtrang')-- quyền menu

	IF(@bcot1 = 1)
	BEGIN
		--quyền menu
		SELECT TOP 1  A.STT,A.SO_PHIEU,A.NGAY_KT,A.NGAY_KH,A.MUC_UU_TIEN, A.NHAN_XET,A.USERNAME AS USERNAME,A.HOAN_THANH,@icot2 QUYEN_MENU, 0 AS THEM
		FROM dbo.GIAM_SAT_TINH_TRANG A
		WHERE  A.SO_PHIEU = @sCot1 
	END
	ELSE
    BEGIN
		SELECT -1 AS STT,dbo.AUTO_CREATE_SO_PHIEU_GSTT(GETDATE()) SO_PHIEU,GETDATE() AS NGAY_KT,GETDATE() AS NGAY_KH,NULL MUC_UU_TIEN, NULL AS NHAN_XET, @UserName AS USERNAME,CONVERT(BIT,0)AS HOAN_THANH,@icot2 QUYEN_MENU,1 AS THEM
	END
END
...
BEGIN
        ...
        SELECT  DISTINCT
						...
						CONVERT(NVARCHAR(500),'') AS [Path]
						INTO #DATAMORNINGTORING
                FROM    THONG_SO_GSTT
                ...
                WHERE   ( CAU_TRUC_THIET_BI_TS_GSTT.MS_MAY = @deviceID )
						...
						AND (DOI_SP = (@isDue - 1) OR @isDue = 0 OR @isDue = 1)
	...
	--định lượng
                ...
                GROUP BY THONG_SO_GSTT.TEN_TS_GSTT ,TEN_TS_GSTT_ANH,TEN_TS_GSTT_HOA,
						...
						SET A.Measurement =1,
						A.Note = B.GHI_CHU
						FROM #DATAMORNINGTORING A
						INNER JOIN dbo.GIAM_SAT_TINH_TRANG_TS_DT B ON A.DeviceID = B.MS_MAY AND A.MonitoringParamsID = B.MS_TS_GSTT AND A.ComponentID = B.MS_BO_PHAN
						AND A.ValueParamID = B.STT_GT 
						WHERE A.TypeOfParam = 1 AND B.STT = @icot1
						...
						A.Note = B.GHI_CHU
						FROM #DATAMORNINGTORING A
						INNER JOIN dbo.GIAM_SAT_TINH_TRANG_TS B ON A.DeviceID = B.MS_MAY AND A.MonitoringParamsID = B.MS_TS_GSTT AND A.ComponentID = B.MS_BO_PHAN
						WHERE A.TypeOfParam = 0 AND B.STT = @icot1


						UPDATE A
						SET A.[Path] = B.DUONG_DAN
						FROM #DATAMORNINGTORING A
						INNER JOIN dbo.GIAM_SAT_TINH_TRANG_HINH B ON A.DeviceID = B.MS_MAY AND A.MonitoringParamsID = B.MS_TS_GSTT AND A.ComponentID = B.MS_BO_PHAN 
						WHERE B.STT = @icot1
						...
						BEGIN
							...
							SELECT * FROM #DATAMORNINGTORING A
							WHERE EXISTS (
							SELECT * FROM (SELECT DISTINCT DeviceID,MonitoringParamsID,ComponentID,TypeOfParam FROM #DATAMORNINGTORING ) B WHERE  A.DeviceID = B.DeviceID AND A.MonitoringParamsID = B.MonitoringParamsID AND A.ComponentID = B.ComponentID AND A.TypeOfParam = B.TypeOfParam
							)
							END
						ELSE
                        BEGIN
							SELECT * FROM #DATAMORNINGTORING
						END
...
BEGIN
	...
	ELSE
    BEGIN
		UPDATE dbo.GIAM_SAT_TINH_TRANG 
		SET NGAY_KH = @dCot1,
        USERNAME =@UserName,
		MUC_UU_TIEN = @icot1,
		NHAN_XET = @sCot1
	...
	END
		...
		DELETE dbo.GIAM_SAT_TINH_TRANG_HINH WHERE STT = @stt
		DELETE dbo.GIAM_SAT_TINH_TRANG_TS_DT WHERE STT = @stt
		DELETE dbo.GIAM_SAT_TINH_TRANG_TS WHERE STT = @stt
	
		IF NOT EXISTS(SELECT * FROM #BTGS)
		BEGIN
			  INSERT INTO dbo.GIAM_SAT_TINH_TRANG_TS( STT ,MS_MAY ,MS_TS_GSTT ,MS_BO_PHAN ,MS_TT ,THOI_GIAN ,TG_TT)
			 SELECT @stt,@deviceID,MS_TS_GSTT,MS_BO_PHAN,MS_TT,THOI_GIAN,THOI_GIAN FROM dbo.CAU_TRUC_THIET_BI_TS_GSTT WHERE MS_MAY = @deviceID
		END
		ELSE
        BEGIN
			INSERT INTO dbo.GIAM_SAT_TINH_TRANG_TS( STT ,MS_MAY ,MS_TS_GSTT ,MS_BO_PHAN ,MS_TT ,THOI_GIAN ,TG_TT,GIA_TRI_DO,GHI_CHU)
			SELECT DISTINCT @stt,DeviceID,MonitoringParamsID ,ComponentID ,ID  ,B.THOI_GIAN,B.THOI_GIAN,Measurement,(SELECT TOP 1 Note FROM #BTGS C WHERE C.DeviceID = B.MS_MAY AND C.MonitoringParamsID =B.MS_TS_GSTT AND C.ComponentID = B.MS_BO_PHAN AND C.ID = B.MS_TT)  FROM #BTGS A
			...
			SELECT DISTINCT @stt,DeviceID,MonitoringParamsID ,ComponentID ,ID,REPLACE(DUONG_DAN,'192.168.2.8','115.79.29.70') , Note FROM #BTGS 
			SELECT TOP 1 @deviceID = MS_MAY FROM dbo.GIAM_SAT_TINH_TRANG_TS WHERE STT = @stt
		END


		COMMIT TRANSACTION UpGSTT
		SELECT 1 AS MA, (SELECT SO_PHIEU FROM dbo.GIAM_SAT_TINH_TRANG WHERE STT =@stt) AS [NAME]
		END TRY
		BEGIN CATCH  
...
SELECT TOP 1 A.USERNAME AS UserName,D.HO + ' '+D.TEN  AS FullName,B.TEN_TO AS Department,C.GROUP_NAME AS [Group],A.USER_MAIL AS  Email,D.MS_CONG_NHAN,ISNULL(A.AVATAR,'') AS HINH_CN,C.HT_MAY_CV  FROM dbo.USERS A
...
SELECT TEN_CTY_TIENG_VIET AS TEN_CTY,TEN_NGAN_TIENG_VIET AS TEN_NGAN,DIA_CHI_VIET AS  DIA_CHI,Phone AS PHONE,@username AS [USER_NAME],Fax AS FAX 
,@TEN_TO AS TEN_TO,@TEN_DV AS TEN_DV  ,@HO_TEN AS HO_TEN,REPLACE(DUONG_DAN_TL,'115.79.29.70','192.168.2.8') AS DUONG_DAN_TL,ISNULL(LUU_FILE,0)LUU_FILE,ISNULL(USER_FTP,'')USER_FTP,ISNULL(PASS_FTP,'')PASS_FTP,ISNULL(PORT_FTP,0)PORT_FTP,ISNULL(HOST_FTP,'')HOST_FTP
...
IF(@stt = 1)
BEGIN
	SELECT  A.STT,A.MS_YEU_CAU,B.MS_MAY,B.MO_TA_TINH_TRANG,B.YEU_CAU,B.MS_UU_TIEN,B.MS_NGUYEN_NHAN,A.NGAY,A.GIO_YEU_CAU,CASE WHEN B.THOI_GIAN_DSX IS NULL THEN 0 ELSE 1 END AS DUYET,
	...
	SELECT STT,
           MS_MAY,
		   dbo.AUTO_CREATE_SO_PHIEU_YC(GETDATE()) AS MS_YEU_CAU,
           MO_TA_TINH_TRANG,
           YEU_CAU,
           MS_UU_TIEN,
           MS_NGUYEN_NHAN,
           ...
           NGAY_XAY_RA,
           HONG,
           REPLACE(Files,'115.79.29.70','192.168.2.8') AS Files,
           USERNAME,
...
BEGIN
	SELECT REPLACE(DUONG_DAN,'115.79.29.70','192.168.2.8') AS  DUONG_DAN  FROM dbo.YEU_CAU_NSD_CHI_TIET_HINH  WHERE MS_MAY = @deviceID AND STT = @icot1  FOR JSON AUTO
END
...
--spCMMSWeb
		...
		(   @stt,         -- STT - int
		  ...
		  DECLARE  @stt1 INT
			...
			SELECT @stt,@deviceID,@stt1, REPLACE(DUONG_DAN,'192.168.2.8','115.79.29.70')  FROM #BTHINH
		...
		SELECT 1 AS MA, (SELECT TOP 1 SO_YEU_CAU FROM dbo.YEU_CAU_NSD WHERE STT = @stt) AS [NAME]
...
BEGIN 
		--kiểm tra user đó có quyền trên menu phiếu bảo trì không
	SET @icot1  = (SELECT TOP 1 GROUP_ID FROM dbo.USERS WHERE USERNAME = @UserName)
	SET @icot2 = (SELECT COUNT(*) FROM dbo.NHOM_MENU WHERE GROUP_ID = @icot1 AND MENU_ID = 'MnuPhieuBaoTri')-- quyền menu
	SET @icot3 = (SELECT COUNT(*) FROM dbo.USER_CHUC_NANG WHERE STT = 7 AND USERNAME = @UserName)-- quyền chức năng

	IF(@bcot1 = 1)
	BEGIN
	
		--quyền menu
		SELECT  MS_PHIEU_BAO_TRI,A.NGAY_LAP,A.NGAY_BD_KH,NGAY_KT_KH,MS_LOAI_BT,(SELECT TOP 1 HU_HONG FROM dbo.LOAI_BAO_TRI WHERE MS_LOAI_BT = A.MS_LOAI_BT) HU_HONG,A.MS_UU_TIEN, B.MO_TA_TINH_TRANG AS N'TINH_TRANG_MAY',A.USERNAME_NGUOI_LAP AS USERNAME,@icot2 QUYEN_MENU,@icot3 AS QUYEN_CN,A.GHI_CHU, 0 AS THEM
	...
	ELSE
    BEGIN
		SELECT dbo.AUTO_CREATE_SO_PHIEU_BT(GETDATE()) MS_PHIEU_BAO_TRI,GETDATE() AS NGAY_LAP,GETDATE() AS NGAY_BD_KH,GETDATE() AS NGAY_KT_KH,NULL MS_LOAI_BT,NULL MS_UU_TIEN, NULL AS N'TINH_TRANG_MAY', @UserName AS USERNAME,@icot2 QUYEN_MENU,@icot3 AS QUYEN_CN,'' AS  LY_DO_BT , 1 AS THEM
	END
END
...
BEGIN 
		...
		SELECT TOP 1  MS_PHIEU_BAO_TRI,A.NGAY_LAP,A.NGAY_BD_KH,NGAY_KT_KH,MS_LOAI_BT,(SELECT TOP 1 HU_HONG FROM dbo.LOAI_BAO_TRI WHERE MS_LOAI_BT = A.MS_LOAI_BT) HU_HONG,A.MS_UU_TIEN, B.MO_TA_TINH_TRANG AS N'TINH_TRANG_MAY',@UserName AS USERNAME,@icot2 QUYEN_MENU,@icot3 AS QUYEN_CN,A.GHI_CHU, 0 AS THEM
		FROM dbo.PHIEU_BAO_TRI A
		LEFT JOIN dbo.YEU_CAU_NSD_CHI_TIET B ON A.MS_PHIEU_BAO_TRI = B.MS_PBT
...
BEGIN
	...
	END
		...
		VALUES(@sCot1,@icot3 + 1,@deviceID,@icot1,CONVERT(DATE,@dCot1),@dCot1,@sCot4,@icot2,@dCot2,@dCot3,(SELECT MS_CONG_NHAN FROM dbo.USERS WHERE USERNAME = @UserName),@UserName,@sCot1,@MS_TT_CT,GETDATE(),-1)
	...
	ELSE
	BEGIN
		UPDATE dbo.PHIEU_BAO_TRI SET GHI_CHU = @sCot4,NGAY_BD_KH = @dCot2,NGAY_KT_KH = @dCot3 WHERE MS_PHIEU_BAO_TRI = @sCot1
	END
	SELECT 1 AS MA, CONVERT(NVARCHAR(50),NULL) AS [NAME]
...
IF(@sDanhMuc = 'GET_WORDORDER_DETAILS')
	BEGIN
		...
		B.SL_TT ,REPLACE(CTTBCV.PATH_HD,'115.79.29.70','192.168.2.8') AS PATH_HD,A.PATH_IMAGE
...
IF(@sDanhMuc ='GET_NGUOI_THUC_HIEN_GS')
BEGIN
	...
	SET A.CHON = 1
	FROM #GETNTHGS A
	INNER JOIN dbo.GIAM_SAT_TINH_TRANG_NV B ON B.MS_CONG_NHAN = A.MS_CONG_NHAN 
	WHERE B.STT_GSTT = @stt
...
BEGIN
	...
	BEGIN
		...
		BEGIN
		--spcmmsweb
		SET @sCot2 = @sCot2 +(SELECT TOP 1 B.USERNAME FROM #NGUOITB A INNER JOIN  dbo.USERS B ON A.MS_CONG_NHAN = B.MS_CONG_NHAN WHERE ID = @rownum) +';'
			--EXEC dbo.spSentThongBao @ID = 0,          -- bigint
	...
	SELECT 1 AS MA, ISNULL(@sCot2,'') AS [NAME]
...
IF(@sDanhMuc ='SAVE_NGUOI_THU_HIEN_GS')
BEGIN
	BEGIN TRANSACTION UpNguoiTHGS
	BEGIN TRY
	
	SELECT DISTINCT MS_CONG_NHAN INTO #NGUOITHGS FROM OPENJSON(@json) 
	WITH (MS_CONG_NHAN NVARCHAR(50) '$.MS_CONG_NHAN');

	DELETE dbo.GIAM_SAT_TINH_TRANG_NV WHERE STT_GSTT = @stt

	INSERT INTO dbo.GIAM_SAT_TINH_TRANG_NV
	(
	    STT_GSTT,
	    USERNAME,
	    FULL_NAME,
	...
	SELECT  @stt,         -- STT_GSTT - int
	    (SELECT TOP 1 USERNAME FROM dbo.USERS WHERE MS_CONG_NHAN = A.MS_CONG_NHAN),       -- USERNAME - nvarchar(50)
	    (SELECT TOP 1 HO + ' ' + TEN FROM dbo.CONG_NHAN WHERE MS_CONG_NHAN = A.MS_CONG_NHAN),       -- FULL_NAME - nvarchar(50)
	    MS_CONG_NHAN,       -- MS_CONG_NHAN - nvarchar(9)
	    GETDATE(), -- TU_NGAY_GIO - datetime
	    GETDATE()  -- DEN_NGAY_GIO - datetime
	    FROM  #NGUOITHGS A
	
	SELECT 1 AS MA, CONVERT(NVARCHAR(50),NULL) AS [NAME]

	COMMIT TRANSACTION UpNguoiTHGS
	END TRY
	BEGIN CATCH  
		ROLLBACK TRANSACTION UpNguoiTHGS;   
		SELECT 0 AS MA, ERROR_MESSAGE() AS [NAME]
	END CATCH
END
...
IF(@sDanhMuc = 'COMPLETE_GSTT')
BEGIN
	BEGIN TRY
		SET @deviceID = (SELECT TOP 1 MS_MAY FROM dbo.GIAM_SAT_TINH_TRANG_TS WHERE STT = @stt)
		...
		BEGIN
			...
			BEGIN
				...
				Y_KIEN_DSX = 'Auto',
				THUC_HIEN_DSX = 1
				WHERE STT =@icot2 AND MS_MAY = @deviceID
			END
		END

		UPDATE dbo.GIAM_SAT_TINH_TRANG SET HOAN_THANH = 1 WHERE STT = @stt
		UPDATE dbo.GIAM_SAT_TINH_TRANG_NV SET DEN_NGAY_GIO = GETDATE() WHERE STT_GSTT = @stt
		SELECT 1 AS MA, CONVERT(NVARCHAR(50),NULL) AS [NAME]
	END TRY
	BEGIN CATCH
			SELECT 0 AS MA, ERROR_MESSAGE() AS [NAME]
...
IF(@sDanhMuc = 'UPDATE_IMAGE_PBT')
BEGIN
	update PHIEU_BAO_TRI_CONG_VIEC set PATH_IMAGE = @sCot3 where MS_PHIEU_BAO_TRI = @sCot1 and MS_BO_PHAN = @sCot2 and MS_CV = @icot1
END
...
(   @sCot1, -- MS_PHIEU_BAO_TRI - nvarchar(20)
	...
	--kiểm tra có tự động lock không
	IF EXISTS(SELECT * FROM dbo.THONG_TIN_CHUNG WHERE KHOA_PBT = 1)
	BEGIN
		UPDATE dbo.PHIEU_BAO_TRI SET TINH_TRANG_PBT = 5,
		MS_TT_CT = (SELECT TOP 1 MS_TT_CT FROM TINH_TRANG_PBT_CT WHERE MAC_DINH = 1 AND STT = 4),
		TT_SAU_BT = @sCot2,
		TT_SAU_BT_ANH =@sCot2,
		NGAY_NGHIEM_THU =@dCot2,
		NGUOI_NGHIEM_THU = (SELECT MS_CONG_NHAN FROM dbo.USERS WHERE USERNAME =@UserName),
		USERNAME_NGUOI_NT = @UserName,
		UPDATE_NGAY_CUOI = @dCot2
		WHERE MS_PHIEU_BAO_TRI =@sCot1

		UPDATE A
		SET NGAY_CUOI=@dCot2
		FROM  MAY_LOAI_BTPN  A
		INNER JOIN dbo.PHIEU_BAO_TRI B ON B.MS_LOAI_BT = A.MS_LOAI_BT AND B.MS_MAY = A.MS_MAY AND B.MS_PHIEU_BAO_TRI = @sCot1
		WHERE B.MS_PHIEU_BAO_TRI = @sCot1
	END
	ELSE
    BEGIN
...
IF(@sDanhMuc = 'DELETE_GSTT')
BEGIN
	BEGIN TRANSACTION DWorkGSTT
	BEGIN TRY
		
		--kiểm tra user có phải user tạo cho phiếu bảo trì không
		IF EXISTS(SELECT * FROM GIAM_SAT_TINH_TRANG_NV WHERE STT_GSTT = @stt)
		BEGIN
			SELECT 0 AS MA, CASE @NNgu WHEN 0 then N'Giám sát ngày đã có người thực hiện,Không thể xóa!' ELSE 'Monitoring date has been performed by someone, Cannot be deleted!' END	 AS [NAME]
			ROLLBACK TRANSACTION DWorkGSTT;   
			RETURN;
		END
		...
		DELETE dbo.GIAM_SAT_TINH_TRANG_TS_DT WHERE STT = @stt
		DELETE dbo.GIAM_SAT_TINH_TRANG_TS WHERE STT = @stt
		DELETE dbo.GIAM_SAT_TINH_TRANG WHERE STT = @stt
		
	

		SELECT 1 AS MA, CONVERT(NVARCHAR(50),NULL) AS [NAME]
	COMMIT TRANSACTION DWorkGSTT
	END TRY
	BEGIN CATCH  
		ROLLBACK TRANSACTION DWorkGSTT;   
		SELECT 0 AS MA, ERROR_MESSAGE() AS [NAME]

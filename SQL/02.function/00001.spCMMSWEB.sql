...
ALTER PROCEDURE [dbo].[spCMMSWEB]
	...
	@CoAll INT = 0,
	@fcot1 FLOAT = 0,
	@sCot1 NVARCHAR(500) = 'WO-202302000014',
	@sCot2 NVARCHAR(500) = '01',
	@sCot3 NVARCHAR(500) = '',
	@MS_BP INT = 14,
	@dCot1 DATETIME = '02/07/2022',
	@dCot2 DATETIME = '02/07/2023',
	@json NVARCHAR(MAX) = N'[{"CHON":false,"MS_MAY":"GEL-1501","TEN_MAY":"Bảo trì chung (Các hệ thống khác-Ngoài MMTB SX)","NGAY_CHUYEN":"10/02/2023 10:29:56","KHO_DI":"GF-03-06","TEN_KHO_DI":"Bên ngoài nhà xưởng Bình Định","KHO_DEN":"GF-03-06","TEN_KHO_DEN":"Bên ngoài nhà xưởng Bình Định","NGAY_NHAN":null,"SO_NGAY":0}]',
	@deviceID NVARCHAR(50) ='BAF-1009',
	@isDue INT = 1,
    @stt INT = -1
	--,
...
SELECT DISTINCT MS_MAY,TEN_MAY INTO #MAY_USER FROM [dbo].[MGetMayUserNgay](@DNgay,@UserName,@MsNXuong,-1,-1,'-1','-1',@deviceID,@NNgu)

SELECT DISTINCT T1.MS_MAY,T2.TEN_MAY,CASE WHEN CONVERT(DATE,T1.NGAY_KT_KH ) < CONVERT(DATE,GETDATE()) THEN	1 ELSE 0 END TREBT  INTO #PBT FROM dbo.PHIEU_BAO_TRI T1 INNER JOIN #MAY_USER T2 ON T1.MS_MAY = T2.MS_MAY WHERE (CONVERT(DATE, NGAY_BD_KH) BETWEEN	DATEADD(YEAR,-1,@DNgay) AND CONVERT(DATE,@DNgay)) AND (TINH_TRANG_PBT =2) AND  (T1.MS_MAY = @deviceID OR @deviceID = '-1')

SELECT DISTINCT MS_MAY,TEN_MAY,CASE WHEN CONVERT(DATE,MAX(NGAY_KE)) < CONVERT(DATE,GETDATE()) THEN	1 ELSE 0 END TREGS INTO #GSTT FROM [dbo].[MGetHieuChuanKeGSTT](DATEADD(YEAR,-1,@DNgay),@DNgay,@UserName,@MsNXuong,-1,-1,'-1','-1',-1,1,@NNgu) T1 WHERE (T1.MS_MAY = @deviceID OR @deviceID = '-1')
GROUP BY MS_MAY,TEN_MAY
--[spCMMSWEB]

SELECT DISTINCT B.MS_MAY,C.TEN_MAY,A.NGAY, B.MS_UU_TIEN AS TREYC INTO #YC FROM  dbo.YEU_CAU_NSD A
INNER JOIN dbo.YEU_CAU_NSD_CHI_TIET B ON B.STT = A.STT 
INNER JOIN #MAY_USER C ON C.MS_MAY = B.MS_MAY 
WHERE (B.MS_MAY = @deviceID OR @deviceID = '-1') AND (A.MS_N_XUONG= @MsNXuong OR @MsNXuong = '-1') AND (CONVERT(DATE,A.NGAY) BETWEEN DATEADD(YEAR,-1,GETDATE()) AND CONVERT(DATE,GETDATE())) AND (B.THOI_GIAN_DSX IS NULL OR B.NGAY_DBT IS NULL)
...
LEFT JOIN #YC T4 ON T1.MS_MAY = T4.MS_MAY
 ...
 SELECT CN.MS_MAY,
      ...
      ISNULL(CN.TREYC,0) TREYC,
              ...
              ISNULL(CN.TREGS,0)   TREGS FROM #DSECOMAINT CN order by CN.MS_MAY
 END
 ELSE
 BEGIN
                ...
                IF @stt = -1 -- thêm thì lấy mới
                    BEGIN
                        SELECT  DeviceID,
                                ...
                                Measurement,
                                ValueParamID,
                                Note,CONVERT(IMAGE,NULL) AS ImageGS
                        FROM    #TEMPTB
                    END
                ELSE
                    BEGIN
...
BEGIN
	...
	SELECT DISTINCT DeviceID, MonitoringParamsID, ComponentID,TypeOfParam,ValueParamID,ID,Measurement,Note INTO #BTGS FROM OPENJSON(@json) 
	WITH (DeviceID NVARCHAR(30) '$.DeviceID',	MonitoringParamsID NVARCHAR(10) '$.MonitoringParamsID', ComponentID NVARCHAR(50) '$.ComponentID',
	TypeOfParam BIT '$.TypeOfParam',ValueParamID INT '$.ValueParamID',ID INT '$.ID',Measurement FLOAT '$.Measurement', Note NVARCHAR(250) '$.Note');
...
,@TEN_TO AS TEN_TO,@TEN_DV AS TEN_DV  ,@HO_TEN AS HO_TEN
FROM dbo.THONG_TIN_CHUNG
END
IF(@sDanhMuc ='GetUserRequest')
BEGIN
	SELECT TOP 1 A.STT,B.MS_MAY,B.MO_TA_TINH_TRANG,B.YEU_CAU,B.MS_UU_TIEN,B.MS_NGUYEN_NHAN,A.NGAY,A.GIO_YEU_CAU,CASE B.THOI_GIAN_DSX WHEN NULL THEN 0 ELSE 1 END AS DUYET,
	CONVERT(NVARCHAR(5),A.GIO_YEU_CAU,108)  + ' ' + CONVERT(NVARCHAR(10),A.NGAY,105) + ' '+ A.NGUOI_YEU_CAU AS NGUOI_YEU_CAU,B.NGAY_XAY_RA, CASE ISNULL(B.NGAY_XAY_RA,'') WHEN '' THEN 0 ELSE 1 END AS HONG,(SELECT DUONG_DAN  FROM dbo.YEU_CAU_NSD_CHI_TIET_HINH  WHERE MS_MAY = @deviceID AND STT = @icot1  FOR JSON AUTO ) AS DUONG_DAN
...
BEGIN
		...
		DECLARE  @stt1 INT
			...
			WITH (DUONG_DAN NVARCHAR(250) '$.DUONG_DAN');
...
BEGIN
	...
	IF EXISTS(SELECT * FROM dbo.PHIEU_BAO_TRI_CONG_VIEC_PHU_TUNG_CHI_TIET)
	BEGIN
	...
	UPDATE PHIEU_BAO_TRI SET TT_SAU_BT=NULL,NGAY_NGHIEM_THU=NULL,NGUOI_NGHIEM_THU=NULL WHERE MS_PHIEU_BAO_TRI= @sCot1
